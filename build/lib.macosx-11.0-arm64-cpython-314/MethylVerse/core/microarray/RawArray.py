...
from multiverse_cache import get_data_file, download_dataset
from .illumina_normalize import control_illumina_single, bgcorrect_illumina, calculate_detection_pvalues
...
def read_all_idats(filenames: List[str],
    ...
    meth = pd.DataFrame(np.full((len(names), len(probe_names)), np.nan),
                            index = names,
                            columns = probe_names)

    unmeth = pd.DataFrame(np.full((len(names), len(probe_names)), np.nan),
                            index = names,
                            columns = probe_names)

    snps_unmeth = pd.DataFrame(np.full((len(names), len(snp_names)), np.nan),
                        index = names,
                        columns = snp_names)
    snps_meth = pd.DataFrame(np.full((len(names), len(snp_names)), np.nan),
                        index = names,
                        columns = snp_names)

    if return_pvalues:
        unique_probe_names = total_ids.loc[~total_ids.index.duplicated()]
        unique_probe_names = unique_probe_names.loc[~pd.Index(unique_probe_names.values).duplicated()]
        pvalues = pd.DataFrame(np.full((len(names), len(unique_probe_names.values)), np.nan),
                    index = names,
                    columns = unique_probe_names.values)
    ...
    for k, name in enumerate(names):
        ...
        if return_pvalues:
            sample_pvalues = calculate_detection_pvalues(idat_grn, idat_red, controls)
            common_ids = sample_pvalues.index.intersection(unique_probe_names.index).values
            pvalues.loc[name, unique_probe_names.loc[common_ids].values] = sample_pvalues.loc[common_ids].values
    ...
    # Match pvalues
    if return_pvalues:
        pvalues = pvalues.loc[:,meth.columns.intersection(pvalues.columns)]
        return meth, unmeth, snps_meth, snps_unmeth, pvalues
    else:
...
def multiprocess_read_all_idats(files: Sequence,
    ...
    if n_jobs == 1:
        for arraytype in arraytype_file_pairs.keys():
            ...
            total_pvalues = combine_methylation_values(total_pvalues, results[4], previous_array_type, arraytype)
        ...
        for arraytype in arraytype_file_pairs.keys():
            ...
            pvalues_values = pd.concat(pvalues, axis=0)
            # Combine results
            ...
            total_pvalues = combine_methylation_values(total_pvalues, pvalues_values, previous_array_type, arraytype)

            previous_array_type = arraytype
        
    return total_meth, total_unmeth, total_snp_meth, total_snp_unmeth, total_pvalues, array_types
...
class RawArray(object):
    ...
    def __init__(self, 
        ...
        # load information from h5 file
            ...
            if os.path.isfile(data) and data.endswith(".h5"):
                ...
                self.meth, self.unmeth, self.snps_meth, self.snps_unmeth, self.pvalues, self.array_type = multiprocess_read_all_idats(data, n_jobs=n_jobs)
    ...
    def get_pvalues(self,
        ...
        # Calculate intervals
        if return_intervals:
            pos_cn = cpg_to_position(self.pvalues.columns.values,
                                     genome_version=self.genome_version)

            sorted_chroms = sort_chrom_names(pos_cn.index.unique_labels)
            pos_cn = pos_cn.loc[sorted_chroms,:]

            pos_cn.df = self.pvalues.T.loc[pos_cn.df.loc[:,"cpg_name"].values,:]
    ...
    def sample_pvalues(self):
        ...
        pvalues = np.nanmedian(self.pvalues.values, axis=1)
        pvalues = pd.Series(pvalues, index=self.pvalues.index)

        return pvalues
    

    def filter_pvalues(self,
                       pvalue_threshold: float = 0.01):
        """
        """

        selected = self.pvalues < pvalue_threshold
        self.meth = self.meth.where(selected, other=np.nan)
        self.unmeth = self.unmeth.where(selected, other=np.nan)
        self.pvalues = self.pvalues.where(selected, other=np.nan)
