...
def read_methyldackel(file_names: np.ndarray,
    ...
    cdef np.ndarray[np.intp_t, ndim = 1] sp_indices
    cdef np.ndarray[DTYPE_t, ndim = 1] sp_values
    cdef np.ndarray[DTYPE_t, ndim = 1] dense_values
    ...
    for i, file in enumerate(file_names):
        ...
        if use_sparse:
            ...
            dense_values = np.full(intervals.size, np.nan, dtype=np.float64)
            dense_values[sp_indices] = sp_values
            df[meth_names[i]] = dense_values
