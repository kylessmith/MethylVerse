...
from ...core.utilities import to_microarray, position_to_cpg
...
def chunkify(data, chunks):
    chunks = max(1, min(int(chunks), len(data)))
    if isinstance(data, pd.DataFrame):
        # Preserve DataFrame index/columns; np.array_split may coerce to ndarray.
        return [data.iloc[idx] for idx in np.array_split(np.arange(len(data)), chunks) if len(idx) > 0]
    return [chunk for chunk in np.array_split(data, chunks) if len(chunk) > 0]

def parallel_impute_status(imputer, data, threshold, n_jobs=-1, chunks=10):
    data_chunks = chunkify(data, chunks)
    if not isinstance(data_chunks[0], pd.DataFrame):
        data_chunks = [pd.DataFrame(chunk, columns=getattr(data, "columns", None)) for chunk in data_chunks]
...
def format_probes(data, impute=True):
    """
    Format for M-PACT classifications by making sure the correct probes are in the DataFrame
    """
...
def intervals_to_probes(betas, impute=False):
    """
    Convert intervals to probe IDs as a pandas DataFrame
    """
    
    cpgs  = position_to_cpg(betas)
    betas.df.index = cpgs
    df = betas.df
    df = df.loc[df.index.values!='',:]
    df = df.loc[~df.index.duplicated(),:]

    df = format_probes(df, impute=impute)
