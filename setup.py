...
dataexts  = (".py",  ".pyx", ".pxd",  ".c",".h", ".pickle", ".h5", ".parquet", ".json", ".txt", ".joblib")
...
extra_compile_args_nonmath_optimized = ['-O3', '-lz']
extra_compile_args_nonmath_debug     = ['-O0', '-g']
extra_link_args_nonmath_optimized    = ['-lz']
    ...
    with open(init_py_path) as f:
        for line in f:
            if line.startswith('__version__'):
                version = ast.literal_eval(ast.parse(line).body[0].value)
                break
        else:
...
ext_module_illumina = declare_cython_extension( "MethylVerse.core.microarray.read_illumina", use_math=False, use_openmp=False , include_dirs=my_include_dirs )
ext_module_methyldackel = declare_cython_extension( "MethylVerse.core.sequencing.read_methyldackel.read_methyldackel", use_math=False, use_openmp=False , include_dirs=my_include_dirs )
ext_module_merge = declare_cython_extension( "MethylVerse.tools.merge_regions.merge_regions", use_math=False, use_openmp=False , include_dirs=my_include_dirs )
...
setup(
    ...
    url = "https://github.com/kylessmith/MethylVerse",
    project_urls = {"Documentation": "https://www.biosciencestack.com/static/MethylVerse/docs/index.html"},
    ...
    provides = ["MethylVerse"],
    keywords = ["cython interval ailist c"],
    ext_modules = my_ext_modules,
    packages = find_namespace_packages(include=["MethylVerse", "MethylVerse.*"]),
    package_data={'MethylVerse': ['*.pxd', '*.pyx', '*.c', '*.h', '*.pickle'],
                    '':['*.pxd', '*.pyx', '*.pickle', '*.h5', '*.c', '*.h', '*.parquet', '*.json', '*.txt', '*.joblib', '*.onnx']},
